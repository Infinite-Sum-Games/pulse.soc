...
INSERT INTO issues (id, title, repourl, url, tags, difficulty, resolved) VALUES
(
  gen_random_uuid(),
  'Add support for group chats',
  (SELECT url FROM repository WHERE name = 'Gochaat: A Chat App in Go'),
...
(
  ...
  (SELECT url FROM repository WHERE name = 'Whodis: Keys, Values and Concurrency'),
...
(
  ...
  (SELECT url FROM repository WHERE name = 'Load-Pulse: Load Testing Tool'),
...
(
  ...
  (SELECT url FROM repository WHERE name = 'NodeGainsDB: Ligthweight In-Memory GraphDB'),
...
(
  ...
  (SELECT url FROM repository WHERE name = 'Notivos-AI'),
...
-- +goose StatementBegin
INSERT INTO issue_claims (id, ghUsername, issue_url, claimed_on, elapsed_on) VALUES
(
  gen_random_uuid(),
  'IAmRiteshKoushik',
  (SELECT url FROM issues WHERE url = 'https://github.com/aadit-n3rdy/gochaat/issues/1'),
  NOW(),
  NOW() + INTERVAL '7 days'
),
(
...
(
  gen_random_uuid(),
  'IAmRiteshKoushik',
  (SELECT url FROM issues WHERE url = 'https://github.com/Astrasv/NodeGainsDB/issues/1'),
  NOW() - INTERVAL '2 days',
  NOW() + INTERVAL '5 days'
);
-- +goose StatementEnd
